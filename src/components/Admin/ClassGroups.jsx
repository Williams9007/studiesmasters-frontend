...
// Both curricula are always offered - GES and Cambridge - regardless of
// whether students of that curriculum have registered yet.
const CURRICULUM_CATALOG = {
  GES: ["Primary 1", "Primary 2", "Primary 3", "Primary 4", "Primary 5", "Primary 6", "JHS 1", "JHS 2", "JHS 3", "SHS 1", "SHS 2", "SHS 3"],
  Cambridge: ["Stage 4", "Stage 5", "Stage 6", "Stage 7", "Stage 8", "Stage 9", "Stage 10", "Stage 11", "Stage 12", "Stage 13"],
};
const sameCurriculum = (a, b) => String(a || "").toLowerCase() === String(b || "").toLowerCase();
...
export default function ClassGroups() {
  ...
  const curricula = useMemo(() => {
    const found = students.map((s) => s.curriculum).filter(Boolean).map((cv) => (/^cambridge$/i.test(cv) ? "Cambridge" : /^ges$/i.test(cv) ? "GES" : cv));
    return [...new Set([...Object.keys(CURRICULUM_CATALOG), ...found])];
  }, [students]);
  const grades = useMemo(() => {
    const fromCatalog = CURRICULUM_CATALOG[form.curriculum] || [];
    const fromStudents = students.filter((s) => sameCurriculum(s.curriculum, form.curriculum)).map((s) => s.grade).filter(Boolean);
    return [...new Set([...fromCatalog, ...fromStudents])];
  }, [students, form.curriculum]);
  ...
  const available = useMemo(() => {
    ...
    return students.filter((student) =>
      sameCurriculum(student.curriculum, form.curriculum) &&
  ...
  const addableStudents = (group) => {
    ...
    return students.filter((student) =>
      sameCurriculum(student.curriculum, group.curriculum) &&
