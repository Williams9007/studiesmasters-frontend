...
export default function LoginPage() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 lg:bg-slate-50">
      {/* Mobile: dark gradient background matching teacher login */}
      <div className="flex min-h-screen flex-col bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 lg:hidden">
        <header className="border-b border-white/10 bg-transparent">
          <div className="mx-auto flex max-w-7xl items-center justify-between gap-2 px-4 py-3">
            ...
            >
              <FaBookOpen className="shrink-0 text-lg text-blue-400" />
              <span className="min-w-0">
                <span className="block truncate text-base font-bold text-white">StudiesMasters</span>
              </span>
            </button>
            <button
              type="button"
              onClick={() => navigate("/")}
              className="inline-flex shrink-0 items-center gap-1.5 rounded-lg px-2.5 py-2 text-sm font-medium text-slate-300 hover:bg-white/10 hover:text-white"
            >
        ...
        <main className="flex flex-1 items-center justify-center px-4 py-6">
          <Card className="w-full max-w-sm border border-white/10 bg-white shadow-2xl">
            <CardContent className="p-6">
              <div className="mb-5 text-center">
                <div className={`mx-auto mb-3 flex h-12 w-12 items-center justify-center rounded-xl text-white shadow-lg ${isStudent ? "bg-blue-600 shadow-blue-200" : "bg-violet-600 shadow-violet-200"}`}>
                  <RoleIcon aria-hidden="true" />
                </div>
                <h2 className="text-xl font-bold text-slate-900">Sign in</h2>
                <p className="mt-1 text-xs text-slate-500">Choose your portal, then use your registered account details.</p>
              </div>

              <form onSubmit={handleLogin} className="space-y-4">
                <div>
                  <span id="role-label" className="mb-1.5 block text-sm font-semibold text-slate-700">I am signing in as</span>
                  <div role="group" aria-labelledby="role-label" className="grid grid-cols-2 gap-1.5 rounded-xl bg-slate-100 p-1.5">
                    <button type="button" onClick={() => setRole("student")} className={`flex min-h-10 items-center justify-center gap-1.5 rounded-lg px-2 text-xs font-bold transition-all duration-300 ${isStudent ? "bg-white text-blue-700 shadow-md" : "text-slate-500 hover:text-slate-800"}`} aria-pressed={isStudent}><FaUserGraduate /> Student</button>
                    <button type="button" onClick={() => setRole("teacher")} className={`flex min-h-10 items-center justify-center gap-1.5 rounded-lg px-2 text-xs font-bold transition-all duration-300 ${!isStudent ? "bg-white text-violet-700 shadow-md" : "text-slate-500 hover:text-slate-800"}`} aria-pressed={!isStudent}><FaChalkboardTeacher /> Teacher</button>
                  </div>
                </div>

                <div>
                  <label htmlFor="login-id" className="mb-1.5 block text-sm font-semibold text-slate-700">Email address or User ID</label>
                  <Input id="login-id" type="text" autoComplete="username" placeholder="you@example.com or SM-ST-..." value={loginId} onChange={(event) => setLoginId(event.target.value)} className="h-10 border-slate-300 focus:ring-blue-600" required />
                </div>
                ...
                <div>
                  ...
                  <div className="relative">
                    <Input id="login-password" type={showPassword ? "text" : "password"} autoComplete="current-password" placeholder="Enter your password" value={password} onChange={(event) => setPassword(event.target.value)} className="h-10 border-slate-300 pr-12 focus:ring-blue-600" required />
                    <button type="button" onClick={() => setShowPassword((visible) => !visible)} className="absolute right-1 top-1 flex h-8 w-8 items-center justify-center rounded-lg text-slate-500 hover:bg-slate-100 hover:text-slate-800" aria-label={showPassword ? "Hide password" : "Show password"} aria-pressed={showPassword}>
                ...
                <Button type="submit" disabled={loading} className={`h-10 w-full rounded-xl text-sm font-semibold text-white ${isStudent ? "bg-blue-600 hover:bg-blue-700" : "bg-violet-600 hover:bg-violet-700"}`}>
